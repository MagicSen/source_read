...
def writeLMDB(datasets, lmdb_path, validation):
    ...
    for count in range(numSample):
        ...
        if "COCO_val" in data[idx]['dataset']:
            mask_all = cv2.imread(
                path_header + 'mask2014/val2014_mask_all_' + img_idx + 'png', 0)
            mask_miss = cv2.imread(
                path_header + 'mask2014/val2014_mask_miss_' + img_idx + 'png', 0)
            # print path_header+'mask2014/val2014_mask_miss_'+img_idx+'png'
        elif "COCO" in data[idx]['dataset']:
            mask_all = cv2.imread(
                path_header + 'mask2014/train2014_mask_all_' + img_idx + 'png', 0)
            mask_miss = cv2.imread(
                path_header + 'mask2014/train2014_mask_miss_' + img_idx + 'png', 0)
            # print path_header+'mask2014/train2014_mask_miss_'+img_idx+'png'
        elif "MPI" in data[idx]['dataset']:
            img_idx = data[idx]['img_paths'][-13:-3];
            # print img_idx
            mask_miss = cv2.imread(
                'dataset/MPI/masks/mask_' + img_idx + 'jpg', 0)
            #mask_all = mask_miss
        ...
        if(width < 68):
            img = cv2.copyMakeBorder(
                img, 0, 0, 0, 68 - width, cv2.BORDER_CONSTANT, value=(128, 128, 128))
            mask_all = cv2.copyMakeBorder(
                mask_all, 0, 0, 0, 68 - width, cv2.BORDER_CONSTANT, value=0)
            mask_miss = cv2.copyMakeBorder(
                mask_miss, 0, 0, 0, 68 - width, cv2.BORDER_CONSTANT, value=0)
        ...
        # note it's writecount instead of count!
        ...
        joints = np.asarray(data[idx]['joint_self']
                            ).T.tolist()  # transpose to 3*16
        ...
        if(nop != 0):
            ...
            for n in range(nop):
                # transpose to 3*16
                joints = np.asarray(joint_other[n]).T.tolist()
                for i in range(len(joints)):
                    row_binary = float2bytes(joints[i])
                    for j in range(len(row_binary)):
                        meta_data[clidx][j] = ord(row_binary[j])
        ...
        if "COCO" in data[idx]['dataset']:
            img4ch = np.concatenate(
                (img, meta_data, mask_miss[..., None], mask_all[..., None]), axis=2)
            #img4ch = np.concatenate((img, meta_data, mask_miss[...,None]), axis=2)
        elif "MPI" in data[idx]['dataset']:
            img4ch = np.concatenate(
                (img, meta_data, mask_miss[..., None]), axis=2)
